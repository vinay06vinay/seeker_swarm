...
public:
  RobotSwarm(std::string node_name, std::string robot_name,
    ...
    goal_pub = this->create_publisher<std_msgs::msg::Bool>("goal_reached", 10);
    robot_pose_curr = this->create_subscription<nav_msgs::msg::Odometry>(
        pose_topic_name, 10,
    ...
    cam_sub_curr = this->create_subscription<sensor_msgs::msg::Image>(
        detection_topic_name, 10,
        std::bind(&RobotSwarm::imageCallback, this, std::placeholders::_1));
    subscriber_move_flag = this->create_subscription<std_msgs::msg::Bool>(
        "/red_object_detected", 10,
        std::bind(&RobotSwarm::move_flag_callback, this,
                  std::placeholders::_1));
    common_publisher_ =
        this->create_publisher<std_msgs::msg::Bool>("/red_object_detected", 10);
  ...
  void move_flag_callback(const std_msgs::msg::Bool::SharedPtr msg) {
    ...
    if (global_move_flag) {
      RCLCPP_INFO(this->get_logger(),
                  "Received true from /red_object_detected. Start moving!");
  ...
  void set_goal(double x, double y) {
    ...
    goal_curr_x = x;
    goal_curr_y = y;
    RCLCPP_INFO_STREAM(this->get_logger(), "Going to goal: ["
                                               << goal_curr_x << ","
                                               << goal_curr_y << "]");
  }
  ...
  void lidar_callback(const sensor_msgs::msg::LaserScan &msg) {
    ...
    for (int i = min_angle; i < min_angle + max_angle; i++) {
      if (scan_data[i % 360] < threshold) {
        // turn
        RCLCPP_INFO_STREAM(this->get_logger(), "********** Obstacle**********");
        obstacle = true;
      } else {
        // move forward
  ...
  void navigate_callback() {
    ...
    if (distance_to_goal > 0.1) {
      ...
      double angle_to_goal = std::atan2(goal_curr_y - curr_loc.second,
    ...
    try {
      ...
      for (const auto &contour : contours) {
        ...
        if (area > contourAreaThreshold) {
          ...
          // cv::drawContours(cv_ptr->image,
          // std::vector<std::vector<cv::Point>>{contour}, -1, cv::Scalar(0,
          // 255, 0), 2);

          // Display a message
          RCLCPP_INFO(this->get_logger(), "Contour area: %s",
                      std::to_string(area).c_str());
          RCLCPP_INFO(this->get_logger(),
                      "Red object detected with a large enough area!");

          // Stop further processing or add your desired logic here

          // For example, you can publish a message or set a flag to stop
          // processing
          ...
          // You might want to add a break here to exit the loop if you only
